                       ...
                       char *name, char *type, char *mode)
{
   ...
   /* allocate memory for the new data dictionary element, setting all values
   ** to NULL using the calloc() */
   if ((dictionary_ptr->root = (data_dict_element_struct*)calloc(1,
                                     sizeof(data_dict_element_struct))) == NULL)
   {
      printf("\nERROR:[data_dict_add_root]-Unable to alloc mem for root");
      return FALSE;
   }
                             ...
                             data_dictionary_struct *dictionary_ptr, char* name)
{
   ...
   /* perform a depth first search on the tree starting at the root, with the
   ** element name we are trying to find and a flag stating we have not found
   ** a match */
   return data_dict_dfs_find_element(dictionary_ptr->root, name, &found_match);
}
            ...
            char *parent_name, char *level, char *name, char *type, char *mode)
{
   ...
   if ((parent_ptr = data_dict_get_element(dictionary_ptr,parent_name)) == NULL)
   {
      /* if there is not an entry associated with the parent name then error */
      printf("\nERROR:[data_dict_add_entry]-Unable to find entry for %s",
             parent_name);
      return FALSE;
   }
   ...
   ** to NULL using the calloc() */
   if ((new_element_ptr = (data_dict_element_struct*)calloc(1,
                                     sizeof(data_dict_element_struct))) == NULL)
   {
      printf("\nERROR:[data_dict_add_entry]-Unable to alloc mem for entry");
      return FALSE;
   }
   ...
   if (data_dict_add_child_element(parent_ptr, new_element_ptr) == FALSE)
   {
      printf("\nERROR:[data_dict_add_entry]-Unable to add child element");
      return FALSE;
   }
                            ...
                            char *action_name, char *desc_type, char *desc_text)
{
   ...
   if ((action_ptr = data_dict_get_element(dictionary_ptr, action_name)) ==NULL)
   {
      printf(
        "\nERROR:[data_dict_action_add_desc]-Could not find element in dict %s",
        action_name);
      return FALSE;
   }

   /* search to see if we already have an attribute of the same type */
   temp_attr_ptr = action_ptr->first_attribute_ptr;
   prev_attr_ptr = temp_attr_ptr;
   found_attribute = FALSE;
   while ((temp_attr_ptr != NULL) && (found_attribute == FALSE))
   {
      if (strcmp(temp_attr_ptr->attribute, desc_type) == 0)
      {
   ...
   /* if the attribute type does not exist, then we need to add it to the end
   ** of the attribute list */
   if (found_attribute == FALSE)
   {
      if ((curr_attr_ptr = (data_dict_attribute_list_struct*)calloc(1,
                                        sizeof(data_dict_attribute_list_struct))) == NULL)
      {
         printf("\nERROR:[data_dict_action_add_desc]-Cant alloc mem for %s",
                desc_type);
         return FALSE;
      }
      ...
      if (action_ptr->first_attribute_ptr == NULL)
      {
         ...
         action_ptr->first_attribute_ptr = curr_attr_ptr;
      }
      else
      {
         prev_attr_ptr->next_attribute = curr_attr_ptr;
      }
   }

   /* add the description to the end of the curr_attr_ptr description list */
   ...
   {
      curr_attr_ptr->first_description = new_string_ptr;
   }
   else
   {
      prev_string_ptr->next_string = new_string_ptr;
   }
...
{
   printf("\nINFO:[data_dict_print_to_screen]-This is the data dictionary");
...
** Postcondition:	 A valid pointer into the data dictionary or NULL is returned.
** Description:		 The function traverses the tree to find the next sibling
**			 or its parent's sibling, etc in the tree.  If none are found
...
Boolean data_dict_set_type(data_dict_element_struct* element_ptr, char* type)
{
   ...
   {
      printf(
          "\nERROR:[data_dict_set_mode]-Unable to set type. NULL ptr detected");
      return FALSE;
   }
...
Boolean data_dict_set_mode(data_dict_element_struct* element_ptr, char* mode)
{
   ...
   {
      printf(
          "\nERROR:[data_dict_set_mode]-Unable to set mode. NULL ptr detected");
      return FALSE;
   }
...
Boolean add_level_info_to_data_dictionary(data_dictionary_struct*dictionary_ptr)
{
   ...
   if (dictionary_ptr == NULL)
   {
      printf("\nERROR:[add_level_info_to_data_dictionary]-");
      printf("Unable to add level info.  NULL data dictionary detected");
      return FALSE;
   }
   ...
   {
      printf("\nERROR:[add_level_info_to_data_dictionary]-");
      printf("Unable to add level info.  NULL root detected");
      return FALSE;
   }
                                            ...
                                            data_dict_element_struct *child_ptr)
{
   ...
   {
      printf(
      "\nERROR:[data_dict_add_child_element]-Unable to add child, NULL parent");
      return FALSE;
   }
   ...
   {
      printf(
             "\nERROR:[data_dict_add_child_element]-Unable to allocate memory");
      return FALSE;
   }
                ...
                data_dict_element_struct *element, char* name, int* match_found)
{
   ...
   while (temp_ptr != NULL)
   {
      ...
      result_ptr = data_dict_dfs_find_element(temp_ptr->data_ptr, name,
                                                                   match_found);
...
void data_dict_dfs_print(data_dict_element_struct *element)
{
   ...
   if (element != NULL)
   {
      ...
      while (temp_ptr != NULL)
      {
         /* print out the current child */
/*         data_dict_print_element(temp_ptr->data_ptr);  */
...
void data_dict_print_element(data_dict_element_struct *element)
{
   ...
   if (element != NULL)
   {
      ...
      {
         printf("\n\tAttribute      : %s", temp_attr_ptr->attribute);
