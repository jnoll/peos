...
/**
 * JMainFrame serves as the top level container frame for the PML UI and implements
 * many, non-view, default application behaviors such as menus, file IO, and so forth
 *
 *
                                                  ...
                                                  ActionListener
{
     ...
     * to the super class for proper display.  This constructor creates the split pane,
     * the tree and graph view, and inserts all of the menus, toolbars and other little bells
     * and whistles for the program.
     *
    ...
    public JMainFrame()
    {
        ...
        // the applcation name - note that the title should change to reflect
        // the currently open filename which is handled by the DataView
        super( "" );
...
*****/        
    ...
    /**                                  
     * The file new method is called from the menu (actually through the PMLView)
     * The File New command creates a new PML Process node and attaches it to this
     * model.  From there the user may add or remove as desired.
     *
     */
    public void fileNew()
    {
...
//TODO Setup the following with the proper filter - not available in my swing version right now
//      chooser.setFileFilter( new ExtensionFileFilter( "pmd",
//                                                      "PML Model Files" ) ); 

        String FileToOpen;                  // Storage for the file we are trying to open
        JPMLDataModel DataModel = null;     // Storage for the DataModel we are supposedly reading
        ...
        // if we get here and the data model is valid then we just loaded a valid filename
        if ( DataModel == null )
            return;
        ...
        m_DataView.setDataModel( m_DataModel );
//TODO Insert this when the tree view is built        m_TreeView.setDataModel( m_DataModel );
     ...
     * can interchange model files or restore previous sessions.  This method presents
     * the SaveAs chooser dialog if the parameter flag is set, or if the model is unnamed
     *
...
//TODO Implement file overwrite verification here
        // We got a valid filename here so open a file and generate the PML into then close
        try
        {
            BufferedWriter file = new BufferedWriter( new FileWriter( PMLFilename ) );
