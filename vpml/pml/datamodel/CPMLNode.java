...
abstract public class CPMLNode extends CGraphNode 
{
    ...
    private String CommentsToPML( String Whitespace )
    {
        ...
        for ( int WordStartIndex = 0; WordStartIndex < m_Comments.length(); WordStartIndex++)
        {
            ...
            // If we've exceeded the end of the source string then decrement the WordLen
            // by one so we don't choke trying to remove the substring
            if ( WordLength >= m_Comments.length() )
                WordLength--;
            
            // Now we've got either the end of the Comments, or the end of a word
            // if we've got a word then check the line length plus the word length
            // if we exceed 80 characters then start a new line
            ...
            Word = m_Comments.substring( WordStartIndex, WordStartIndex + WordLength );
            
            // Add the word to the current PML formatted comment string we're building
            Comments += Word;
    ...
    /** 
     * Method to call for outputting the children of this node into the PML string
     * This just recurses the children which recurses their children etc
    ...
    public String ChildrenToPML( String pmlString, int SpaceCardinality )
    {
        ...
        // Otherwise recurse the children of this node.  Add 3 to the space cardinality so
        // all children are indented by three space in scope
