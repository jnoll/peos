...
int main()
{
	...
	printf("Location: handle_run.cgi?resource_type=requires&process_filename=%s&pid=%d&act_name=%s\r\n\r\n",process_filename,pid,action_name);
    }
    
    
    if(strcmp(action_event, "Finish") == 0) {
	printf("Location: handle_run.cgi?resource_type=provides&process_filename=%s&pid=%d&act_name=%s\r\n\r\n",process_filename,pid,action_name);
    }
