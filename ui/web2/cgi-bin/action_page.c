...
void write_content()
{
    ...
    if ((resource_type) && ((strcmp(resource_type,"requires") != 0) ||
		(strcmp(resource_type,"provides") != 0))) { 
	if (strcmp(resource_type,"requires") == 0) {
	    resources = peos_get_resource_list_action_requires(pid, action_name,
			&num_resources);
	} else {
	...
	if (((num_unbound_resources == 0) || (num_resources == 0)) && (action == NULL)) {
	    ...
	    add_action_list(ap);
	    add_jscript();
	    printf("        <h4>Supply Resource Bindings</h4><br>\n");
